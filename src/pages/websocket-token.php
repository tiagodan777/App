...
if ($membroId === '') responderJsonWebSocketToken(['success' => false, 'message' => 'A sessão terminou.'], 401);
...
try {
    ...
    responderJsonWebSocketToken(['success' => true, 'token' => $token, 'expires_in' => 60]);
} catch (Throwable $erro) {
    error_log('[websocket-token] ' . $erro->getMessage());
    responderJsonWebSocketToken(['success' => false, 'message' => 'Não foi possível preparar a ligação.'], 500);
}