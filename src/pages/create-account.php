...
if (isset($_FILES['imagens']['tmp_name']) && is_array($_FILES['imagens']['tmp_name'])) {
    ...
    if ($totalImagens > 6) $erros['imagens'] = 'Podes adicionar no máximo 6 fotografias.';
...
$nomeCompleto = trim($membro['primeiro_nome'] . ' ' . $membro['ultimo_nome']);
$membro['nome_seo'] = create_seo_name($nomeCompleto);

if (!is_array($membro['gostos'])) $membro['gostos'] = [];

$membro['gostos'] = array_values(array_unique(array_filter(array_map(
    static fn($gosto): string => trim((string) $gosto),
    $membro['gostos']
...
$dataNascimentoValida = $dia >= 1
    && $mes >= 1
    && $ano >= 1900
    && $ano <= (int) date('Y')
    && checkdate($mes, $dia, $ano);

$erros['nascimento'] = $dataNascimentoValida ? '' : 'Escolhe uma data de nascimento válida.';
$erros['genero'] = Validate::isGenero($membro['genero']) ? '' : 'Escolhe um género válido.';
...
if ($erros !== []) {
    apagarImagensTemporarias($imagens, $pathImagensTemporarias);
...
try {
    ...
    responderJson([
        'success' => true,
        'redirect' => DOC_ROOT . 'index/?loginToken=' . urlencode((string) $tokenLogin)
    ]);
} catch (\Throwable $erro) {
    apagarImagensTemporarias($imagens, $pathImagensTemporarias);
    error_log('Erro ao criar conta: ' . $erro->getMessage());
