...
if ($metodo === 'POST') {
    $limiteMembro = consumirLimiteRequisicoes('delete-account-member', chaveLimiteRequisicoes($membroId), 3, 60 * 60);
    $limiteIp = consumirLimiteRequisicoes('delete-account-ip', chaveLimiteRequisicoes(enderecoCliente()), 10, 60 * 60);
    if (!$limiteMembro['permitido'] || !$limiteIp['permitido']) {
        $tentarEm = max(1, (int) $limiteMembro['tentar_em'], (int) $limiteIp['tentar_em']);
        ...
        try {
            ...
            if ($token !== '') {
                try {
                    $tokens->delete($token);
                } catch (Throwable $erroToken) {
                    error_log(
                        '[profile-delete] Não foi possível revogar o token após a falha: ' . $erroToken->getMessage()
                    );
                }
            }
...
echo $twig->render('profile-delete.html', ['membro' => $membro, 'sent' => $enviado, 'mensagem_erro' => $mensagemErro]);
