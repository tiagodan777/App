...
if ($metodo === 'POST') {
    /* O index.php já valida CSRF; esta chamada mantém a página segura isoladamente. */
    require_csrf_token();
    ...
    if (!$limiteMembro['permitido'] || !$limiteIp['permitido']) {
        ...
        $mensagemErro =
            'Foram feitos demasiados pedidos. Tenta novamente mais tarde.';
    } else {
        $tokens = $cms->getToken();
        $token = '';
        ...
        try {
            /*
             * Token::create() elimina o token anterior deste propósito e usa
             * uma transação. Só enviamos o email depois de o token existir.
             */
            $token = $tokens->create(
                $membroId,
                'delete_account'
