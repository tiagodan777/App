...
try {
    ...
    if (
        in_array($acao, ['mark_read', 'delete_message', 'react'], true)
        && !$existente
        && !$ligados
    ) {
    ...
    if (
        !$ligados
        && !$access->validProximityToken(
            $membroId,
            $outroId,
            (string) ($_POST['profile_access_token'] ?? '')
        )
    ) {
        json_response([
            'success' => false,
    ...
    if (
        !$ligados
        && !$access->hasReply($membroId, $outroId)
        && $access->sentBeforeReply($membroId, $outroId) >= 2
    ) {
        json_response([
            'success' => false,
    ...
    $replyId = empty($_POST['reply_to'])
        ? null
        : filter_var($_POST['reply_to'], FILTER_VALIDATE_INT);

    if ($replyId === false || ($replyId !== null && $replyId < 1)) {
    ...
    json_response([
        'success' => true,
        'message' => $messages->get($mensagemId, $membroId)
    ], 201);
} catch (InvalidArgumentException $erro) {
    if ($json) {
