...
function handle_shutdown(): void {
    $erro = error_get_last();
    $tiposFatais = [E_ERROR, E_PARSE, E_CORE_ERROR, E_COMPILE_ERROR, E_USER_ERROR, E_RECOVERABLE_ERROR];
    if ($erro === null || !in_array($erro['type'], $tiposFatais, true)) {
        return;
    }
    handle_exception(new ErrorException($erro['message'], 0, $erro['type'], $erro['file'], $erro['line']));
}
