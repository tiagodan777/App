...
final class ProfileAccess {
    ...
    public function canView(string $viewerId, string $profileId): bool {
        ...
        $viewerAgeGroup = Validate::ageGroup((string) $members[$viewerId]['nascimento']);
        $profileAgeGroup = Validate::ageGroup((string) $members[$profileId]['nascimento']);
        if (
            $viewerAgeGroup === null ||
        ...
        return (new MemberConnection($this->db))->areConnected($viewerId, $profileId) ||
    ...
    private function members(string $viewerId, string $profileId): array {
        $rows = $this->db->runSQL(
            'SELECT id, nascimento
                FROM membros
                WHERE id = :viewer_id OR id = :profile_id',
            ['viewer_id' => $viewerId, 'profile_id' => $profileId]
        )->fetchAll();
        $members = [];
        foreach ($rows as $row) {
    ...
    private function areBlocked(string $firstId, string $secondId): bool {
        return (bool) $this->db->runSQL(
            'SELECT 1
                FROM bloqueados
                WHERE ( pessoa_bloqueou_id = :first_1 AND pessoa_bloqueada_id = :second_1 ) OR ( pessoa_bloqueou_id =
                :second_2 AND pessoa_bloqueada_id = :first_2 )
                LIMIT 1',
            ['first_1' => $firstId, 'second_1' => $secondId, 'second_2' => $secondId, 'first_2' => $firstId]
        )->fetchColumn();
    }
    ...
    private function haveConversation(string $firstId, string $secondId): bool {
        return (bool) $this->db->runSQL(
            'SELECT 1
                FROM mensagens_chat
                WHERE ( emissor_id = :first_1 AND destinatario_id = :second_1 ) OR ( emissor_id = :second_2 AND
                destinatario_id = :first_2 )
                LIMIT 1',
            ['first_1' => $firstId, 'second_1' => $secondId, 'second_2' => $secondId, 'first_2' => $firstId]
        )->fetchColumn();
    }
    ...
    private function haveVisibleHey(string $viewerId, string $profileId): bool {
        return (bool) $this->db->runSQL(
            "SELECT 1
                FROM notificacao
                WHERE tipo = 'hey' AND ( ( emissor_id = :viewer_sender AND destinatario_id = :profile_recipient AND
                ocultada_para_emissor_em IS NULL ) OR ( emissor_id = :profile_sender AND destinatario_id =
                :viewer_recipient AND ocultada_para_destinatario_em IS NULL ) )
                LIMIT 1",
            [
    ...
    private function hasProximityPass(string $viewerId, string $profileId): bool {
        return (bool) $this->db->runSQL(
            'SELECT 1
                FROM token
                WHERE membro_id = :profile_id AND proposito = :purpose AND validade > UTC_TIMESTAMP()
                LIMIT 1',
            ['profile_id' => $profileId, 'purpose' => $this->profilePurpose($viewerId)]
        )->fetchColumn();
    }
