...
class WebSocket implements MessageComponentInterface
{
    ...
    private function persistirLocalizacaoParaProximidade(
        ...
        $ultimaPersistencia = (float) (
            $this->ultimaPersistenciaLocalizacaoPorMembro[$membroId]
            ?? 0.0
        );

        if (
            $ultimaPersistencia > 0 &&
            ($agora - $ultimaPersistencia) <
                self::PERSISTENCIA_LOCALIZACAO_INTERVALO_SEGUNDOS
        ) {
            return;
        }
        ...
        try {
            ...
            $nearby = new NearbyPresenceNotification(
                $database,
                new PushNotification($database)
            );

            $posicaoAnterior =
                $nearby->locationSnapshot($membroId);
            ...
            echo sprintf(
                "[LOCATION PERSIST] %s gravado na BD. Visível=%s\n",
                $membroId,
                $visivel ? 'sim' : 'não'
            );
    ...
    private function desconectarMembros(ConnectionInterface $from, array $data): void
    {
        ...
        unset(
            $this->tentativasConexao[
                $this->chaveConexao($remetenteId, $destinatarioId)
            ]
        );
    ...
    private function publicarMensagemChat(ConnectionInterface $from, array $data): void
    {
        ...
        $mensagem['emissor_perfil_url'] =
            '/profile/' .
            rawurlencode((string) $mensagem['emissor_id']);
    ...
    private function publicarEliminacaoChat(ConnectionInterface $from, array $data): void
    {
        ...
        try {
            ...
            $emissorId = trim(
                (string) ($apagada['emissor_id'] ?? '')
            );

            $destinatarioId = trim(
                (string) ($apagada['destinatario_id'] ?? '')
            );
            ...
            foreach (
                ...
                foreach (
                    $this->ligacoesPorMembro[$participanteId] ?? []
                    as $client
                ) {
                    $this->enviar(
                        $client,
    ...
    private function sincronizarLocalizacoesPersistidas(bool $forcar = false): void
    {
        ...
        try {
            ...
            foreach ($statement->fetchAll(PDO::FETCH_ASSOC) as $linha) {
                ...
                if (!$this->membroTemLigacaoVisivel($membroId)) {
                    $foto = basename(
                        trim((string) ($linha['foto_perfil'] ?? 'default.webp'))
                    );
            ...
            foreach (
                ...
                if (isset($visiveisAgora[$membroId])) continue;
    ...
    private function carregarBloqueios(bool $forcar = false): bool
    {
        ...
        if (
            !$forcar &&
            ($agora - $this->bloqueiosCarregadosEm) <
