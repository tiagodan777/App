...
final class Location {
    ...
    public function saveBackground(
        ...
        $this->db->runSQL(
            'INSERT INTO localizacao_membro ( membro_id, latitude, longitude, precisao_m, localizacao_ativa, visivel,
                origem, atualizada_em )
                VALUES ( :member_id, :latitude, :longitude, :accuracy, :active, :visible, :source, UTC_TIMESTAMP() )
                ON DUPLICATE KEY UPDATE latitude = VALUES(latitude), longitude = VALUES(longitude), precisao_m =
                VALUES(precisao_m), localizacao_ativa = VALUES(localizacao_ativa), visivel = VALUES(visivel), origem =
                VALUES(origem), atualizada_em = UTC_TIMESTAMP()',
            [
                'member_id' => $memberId,
                'latitude' => $latitude,
                'longitude' => $longitude,
    ...
    public function disable(string $memberId): void {
        ...
        $this->db->runSQL(
            'UPDATE localizacao_membro
                SET localizacao_ativa = 0, visivel = 0, atualizada_em = UTC_TIMESTAMP()
