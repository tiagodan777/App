...
final class AuthenticatedWebSocket implements MessageComponentInterface
{
    ...
    public function __construct(MessageComponentInterface $aplicacao, callable $pdoFactory, LoopInterface $loop, array $origensPermitidas)
    {
    ...
    public function onClose(ConnectionInterface $conn): void
    {
        ...
        if (!$this->ligacoesAbertas->contains($conn)) return;
    ...
    private function origemPermitida(ConnectionInterface $conn): bool
    {
        ...
        if (!is_object($pedido) || !method_exists($pedido, 'getHeaderLine')) return false;
    ...
    private function normalizarOrigem(string $origem): string
    {
        ...
        if (!in_array($esquema, ['http', 'https'], true) || $host === '') return '';
    ...
    private function agendarLimiteAutenticacao(ConnectionInterface $conn): void
    {
        ...
        $this->temporizadoresAutenticacao[$conn->resourceId] = $this->loop->addTimer(
            ...
            function () use ($conn): void {
                ...
                if (isset($this->ligacoesAutenticadas[$conn->resourceId])) return;
