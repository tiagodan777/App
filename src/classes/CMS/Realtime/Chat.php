...
trait Chat {
    ...
    private function publicarMensagemChat(ConnectionInterface $from, array $data): void {
        ...
        if ($membroId === null) {
            $this->enviar($from, ['type' => 'chat_error', 'message' => 'A ligação não está autenticada.']);
            return;
        }
        if ($mensagemId === false || $mensagemId < 1) {
            $this->enviar($from, ['type' => 'chat_error', 'message' => 'A mensagem não é válida.']);
            return;
        }
        $sql = " SELECT msg.id, msg.emissor_id, msg.destinatario_id, msg.texto, msg.tipo, msg.ficheiro_nome,
            msg.ficheiro_mime, msg.ficheiro_tamanho, msg.lida, msg.criada_em, msg.lida_em, CONCAT(m.primeiro_nome,
            ' ', m.ultimo_nome) AS emissor_nome, COALESCE( ( SELECT fp.nome_arquivo
            ...
            INNER JOIN membros m ON m.id COLLATE utf8mb4_unicode_ci = msg.emissor_id COLLATE utf8mb4_unicode_ci
            WHERE msg.id = :id
            LIMIT 1 ";
        ...
        if (!$mensagem || (string) $mensagem['emissor_id'] !== $membroId) {
            $this->enviar($from, ['type' => 'chat_error', 'message' => 'Não podes publicar esta mensagem.']);
            return;
        }
        $destinatarioId = trim((string) ($mensagem['destinatario_id'] ?? ''));
        if (!$this->interacaoMensagensPermitida($membroId, $destinatarioId)) {
            $this->enviar($from, ['type' => 'chat_error', 'message' => 'Esta conversa não está disponível.']);
            return;
        }
        ...
        $participantes = array_unique([(string) $mensagem['emissor_id'], (string) $mensagem['destinatario_id']]);
    ...
    private function publicarEliminacaoChat(ConnectionInterface $from, array $data): void {
        ...
        try {
            ...
            $statement = $database->prepare('SELECT emissor_id, destinatario_id
                ...
                LIMIT 1');
            $statement->execute(['mensagem' => (int) $mensagemId, 'emissor' => $membroId]);
            ...
            echo sprintf("[CHAT DELETE] Mensagem %d apagada por %s.\n", (int) $mensagemId, $membroId);
    ...
    private function publicarReacaoChat(ConnectionInterface $from, array $data): void {
        ...
        try {
            ...
            $mensagem = $database->prepare('SELECT emissor_id, destinatario_id
                ...
                LIMIT 1');
            $mensagem->execute(['id' => (int) $mensagemId, 'membro1' => $membroId, 'membro2' => $membroId]);
            ...
            $evento = ['type' => 'chat_reaction', 'message_id' => (int) $mensagemId, 'reactions' => $lista];
            $destinatarios = [
    ...
    private function marcarMensagensChatComoLidas(ConnectionInterface $from, array $data): void {
        ...
        if ($leitorId === null || $outroId === '' || $outroId === $leitorId) {
            $this->enviar($from, ['type' => 'chat_error', 'message' => 'A conversa não é válida.']);
            return;
        }
        if (
            !$this->interacaoMensagensPermitida($leitorId, $outroId) ||
            !$this->existeConversaMensagens($leitorId, $outroId)
        ) {
            $this->enviar($from, ['type' => 'chat_error', 'message' => 'Esta conversa não está disponível.']);
            return;
        }
    ...
    private function contarMensagensNaoLidas(string $membroId): int {
        ...
        $statement = $database->prepare(" SELECT COUNT(*)
            ...
            WHERE msg.destinatario_id = :id AND msg.lida = 0 AND {$condicaoFaixaEtaria} AND NOT EXISTS ( SELECT 1
            FROM bloqueados b
        ...
        $statement->execute(['id' => $membroId, 'eu1' => $membroId, 'eu2' => $membroId]);
    ...
    private function interacaoMensagensPermitida(string $primeiroMembroId, string $segundoMembroId): bool {
        ...
        $statement->execute(['primeiro' => $primeiroMembroId, 'segundo' => $segundoMembroId]);
        $membros = [];
        ...
        $primeiraFaixa = $this->obterFaixaEtaria((string) ($membros[$primeiroMembroId]['nascimento'] ?? ''));
        $segundaFaixa = $this->obterFaixaEtaria((string) ($membros[$segundoMembroId]['nascimento'] ?? ''));
