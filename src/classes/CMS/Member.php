...
class Member
{
    ...
    public function prepareAccountForm(array $input, array $sections, bool $creating): array
    {
        ...
        if ($has('nascimento')) {
            if (!Validate::isAdult($data['nascimento'])) {
                $errors['nascimento'] =
                    'Indica uma data válida. Tens de ter pelo menos 18 anos.';
            }
        ...
        if ($has('contactos')) {
            ...
            if (
                $data['telefone'] !== '' &&
                !Validate::isPhone($data['telefone'])
            ) {
                $errors['telefone'] =
        ...
        if ($passwordRequested) {
            if (!Validate::isPassword($data['password'])) {
                $errors['password'] =
                    'Usa pelo menos 8 caracteres, uma maiúscula, uma minúscula e um número.';
            } elseif ($data['password'] !== $confirmation) {
                $errors['confirma_password'] =
        ...
        if ($creating) {
            ...
            if ($value('aceitou_privacidade') !== '1') {
                $errors['aceitou_privacidade'] =
                    'Confirma a Política de Privacidade.';
            }
        }
    ...
    {
        $tastes = $this->normalizarGostos(
            $member['gostos'] ?? []
        );

        $member['email'] = $this->normalizarEmail(
            (string) ($member['email'] ?? '')
        );
         ...
         */
        $member['telefone'] = $this->normalizarTelefone(
            (string) ($member['telefone'] ?? '')
        );
        ...
        try {
            ...
            if (!$id) {
                throw new \RuntimeException(
                    'Não foi possível obter o membro criado.'
                );
            }
    ...
    private function sincronizarGostos(
        ...
        foreach ($tastes as $taste) {
            ...
            if (!$hobbyId) {
                throw new \RuntimeException(
                    'Não foi possível guardar um gosto.'
                );
            }
