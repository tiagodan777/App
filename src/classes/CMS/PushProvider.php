...
final class PushProvider {
    ...
    public function __construct(array $config = []) {
        $this->config = array_replace_recursive(
            [
                'enabled' => false,
                'apns' => ['team_id' => '', 'key_id' => '', 'private_key_file' => '', 'topic' => ''],
                'fcm' => ['service_account_file' => '', 'project_id' => '']
            ],
            $config
        );
    }
    ...
    public function send(array $job): array {
        ...
        return match ((string) ($job['plataforma'] ?? '')) {
            ...
            default => ['success' => false, 'permanent' => true, 'error' => 'unsupported_platform']
        };
    }
    ...
    private function sendApns(array $job): array {
        ...
        $keyFile = $this->requiredConfig($config, 'private_key_file', 'ficheiro da chave APNs');
        $topic = $this->requiredConfig(config: $config, key: 'topic', label: 'APNs topic');
        $deviceToken = trim((string) ($job['token'] ?? ''));
        if ($deviceToken === '') {
            return ['success' => false, 'permanent' => true, 'error' => 'empty_device_token'];
        }
        ...
        $payload = [
            'aps' => [
                'alert' => ['title' => (string) ($job['titulo'] ?? 'Margot'), 'body' => (string) ($job['corpo'] ?? '')],
                'sound' => 'default',
        ...
        $authorization = $this->apnsAuthorizationToken($teamId, $keyId, $keyFile);
        $headers = [
        ...
        $response = $this->apnsRequest($host, $deviceToken, $headers, $json);
        if ($response['network_error'] !== '') {
            return ['success' => false, 'permanent' => false, 'error' => 'apns_network_' . $response['network_error']];
        }
        if ($response['status'] === 200) {
            return ['success' => true, 'permanent' => false, 'error' => '', 'environment' => $environment];
        }
        ...
        $permanentReasons = ['BadDeviceToken', 'DeviceTokenNotForTopic', 'Unregistered'];
        return [
            'success' => false,
    ...
    private function apnsRequest(string $host, string $deviceToken, array $headers, string $json): array {
        return $this->curlRequest($host . '/3/device/' . rawurlencode($deviceToken), $headers, $json, true);
    }

    private function sendFcm(array $job): array {
        $config = $this->config['fcm'] ?? [];
        $serviceAccountFile = $this->requiredConfig($config, 'service_account_file', 'conta de serviço FCM');
        ...
        $data = ['url' => (string) ($job['url'] ?? '/'), 'type' => (string) ($job['tipo'] ?? 'activity')];
        foreach ($job['dados'] ?? [] as $key => $value) {
        ...
        $payload = [
            'message' => [
                ...
                'android' => [
                    ...
                    'ttl' => '86400s',
                    'notification' => [
                        'channel_id' => $this->androidChannelForJob($job),
                        'icon' => 'ic_stat_margot',
                        'sound' => 'default'
                    ]
                ]
        ...
        $json = json_encode($payload, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES | JSON_THROW_ON_ERROR);
        $accessToken = $this->fcmAccessToken($serviceAccount, $serviceAccountFile);
        ...
        if ($response['network_error'] !== '') {
            return ['success' => false, 'permanent' => false, 'error' => 'fcm_network_' . $response['network_error']];
        }
        ...
        $permanent = in_array($providerCode, ['UNREGISTERED', 'SENDER_ID_MISMATCH'], true);
        return [
            'success' => false,
            'permanent' => $permanent,
    ...
    private function apnsAuthorizationToken(string $teamId, string $keyId, string $keyFile): string {
        ...
        $issuedAt = time();
        $unsigned =
            $this->base64UrlEncode(json_encode(['alg' => 'ES256', 'kid' => $keyId], JSON_THROW_ON_ERROR)) .
            '.' .
            $this->base64UrlEncode(json_encode(['iss' => $teamId, 'iat' => $issuedAt], JSON_THROW_ON_ERROR));
        ...
        $token = $unsigned . '.' . $this->base64UrlEncode($this->ecdsaDerToJose($signature, 32));
        $this->apnsTokens[$cacheKey] = ['token' => $token, 'expires_at' => $issuedAt + 3000];
        return $token;
    }

    private function androidChannelForJob(array $job): string {
        return match ((string) ($job['tipo'] ?? 'activity')) {
            'hey' => 'margot_hey',
    ...
    private function fcmAccessToken(array $serviceAccount, string $serviceAccountFile): string {
        if (
        ...
        if ($response['network_error'] !== '') {
            throw new RuntimeException('Falha de rede ao autenticar no FCM: ' . $response['network_error']);
        }
        $body = $this->decodeJsonObject($response['body']);
        $token = trim((string) ($body['access_token'] ?? ''));
        $expiresIn = max(120, (int) ($body['expires_in'] ?? 3600));
        if ($response['status'] !== 200 || $token === '') {
            throw new RuntimeException('O FCM recusou a autenticação (HTTP ' . $response['status'] . ').');
        }
        $this->fcmAccessToken = [
    ...
    private function curlRequest(string $url, array $headers, string $body, bool $http2 = false): array {
        ...
        $networkError = $responseBody === false ? (string) curl_errno($curl) : '';
    ...
    private function requiredConfig(array $config, string $key, string $label): string {
    ...
    private function readJsonFile(string $path): array {
        ...
        try {
            ...
            throw new RuntimeException('O ficheiro da conta de serviço FCM não contém JSON válido.', 0, $error);
        }
        if (!is_array($decoded)) {
    ...
    private function ecdsaDerToJose(string $signature, int $partLength): string {
        ...
        return $this->normaliseEcdsaPart($r, $partLength) . $this->normaliseEcdsaPart($s, $partLength);
    }
