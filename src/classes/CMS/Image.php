...
class Image {
    ...
    public function syncProfilePhotos(string $memberId, array $newPhotos, $requestedOrder, $requestedRemovals): array {
        ...
        try {
            ...
            $rows = $this->db->runSQL(
                'SELECT id, nome_arquivo, status
                    FROM fotos_perfil
                    WHERE membro_id = :id
                    ORDER BY ordem IS NULL, ordem, id FOR UPDATE',
                ['id' => $memberId]
            )->fetchAll();
            $existing = [];
            foreach ($rows as $row) {
            ...
            foreach ($existing as $id => $_) {
                if (!isset($remove[$id]) && !isset($usedExisting[$id])) {
                    $items[] = ['existing', $id];
                }
            }
            foreach ($newPhotos as $index => $name) {
                if (!isset($usedNew[$index])) {
                    $items[] = ['new', $name];
                }
            }
            ...
            foreach ($remove as $id => $_) {
                ...
                $this->db->runSQL('DELETE FROM fotos_perfil WHERE id = :photo AND membro_id = :member', [
                    'photo' => $id,
                    'member' => $memberId
                ]);
            }
            $this->db->runSQL('UPDATE fotos_perfil SET ordem = COALESCE(ordem, 0) + 1000 WHERE membro_id = :id', [
                'id' => $memberId
            ]);
    ...
    public function startProfileWorker(string $memberId): void {
        $worker = APP_ROOT . '/profile-image-worker.php';
        $log = APP_ROOT . '/var/log/profile-image-worker.log';
        if (!is_file($worker)) {
            throw new RuntimeException('O worker das fotografias não existe.');
        }
        ...
        if ($code !== 0) {
            throw new RuntimeException('Não foi possível iniciar o worker das fotografias.');
        }
    }

    public function createImage(string $memberId, string $filename, string $temporary, string $type): void {
        ...
        try {
            ...
            $this->db->runSQL(
                "UPDATE fotos_perfil
                    SET nome_arquivo = :new_name, status = 'completo'
            ...
            if ($source !== $temporary && is_file($source)) {
                @unlink($source);
            }
            if (is_file($temporary)) {
                @unlink($temporary);
    ...
    private function writeWebp(
        ...
        try {
            ...
            if ($enhance) {
                $image->modulateImage(100, 105, 100);
            }
