...
final class NearbyPresenceNotification {
    ...
    public function syncAppState(string $memberId, bool $background): void {
        ...
        $statement = $this->db
            ->prepare('INSERT INTO estado_app_membro ( membro_id, em_background, alerta_proximidade_ativo, total_proximidade,
                atualizado_em )
                VALUES ( :member_id, :background, 0, 0, UTC_TIMESTAMP(6) )
                ON DUPLICATE KEY UPDATE em_background = VALUES(em_background), alerta_proximidade_ativo = CASE WHEN
                VALUES(em_background) = 0 THEN 0 ELSE alerta_proximidade_ativo END, total_proximidade = CASE WHEN
    ...
    public function locationSnapshot(string $memberId): ?array {
        ...
        $statement = $this->db->prepare('SELECT latitude, longitude
            FROM localizacao_membro
            WHERE membro_id = :member_id AND latitude IS NOT NULL AND longitude IS NOT NULL
            LIMIT 1');
    ...
    public function evaluateMember(string $memberId): void {
        ...
        $nearbyCount = $this->nearbyCount($memberId, $location['latitude'], $location['longitude']);
    ...
    private function eligibleBackgroundLocation(string $memberId): ?array {
        $statement = $this->db->prepare(
            'SELECT lm.latitude, lm.longitude
                ...
                INNER JOIN localizacao_membro AS lm ON lm.membro_id COLLATE utf8mb4_unicode_ci = ea.membro_id COLLATE
                utf8mb4_unicode_ci
                INNER JOIN membros AS m ON m.id COLLATE utf8mb4_unicode_ci = ea.membro_id COLLATE utf8mb4_unicode_ci
                WHERE ea.membro_id = :member_id AND ea.em_background = 1 AND lm.localizacao_ativa = 1 AND lm.visivel = 1
                AND lm.latitude IS NOT NULL AND lm.longitude IS NOT NULL AND lm.atualizada_em >= DATE_SUB(
                UTC_TIMESTAMP(), INTERVAL ' .
                self::LOCATION_MAX_AGE_SECONDS .
                ' SECOND
             )
    ...
    private function backgroundCandidatesNear(array $position): array {
        ...
        [$latitudeDelta, $longitudeDelta] = ProximityConfig::boundingBoxDeltas($latitude);
        $statement = $this->db->prepare(
            'SELECT ea.membro_id
                FROM estado_app_membro AS ea
                INNER JOIN localizacao_membro AS lm ON lm.membro_id COLLATE utf8mb4_unicode_ci = ea.membro_id COLLATE
                utf8mb4_unicode_ci
                INNER JOIN membros AS m ON m.id COLLATE utf8mb4_unicode_ci = ea.membro_id COLLATE utf8mb4_unicode_ci
                WHERE ea.em_background = 1 AND lm.localizacao_ativa = 1 AND lm.visivel = 1 AND lm.latitude IS NOT NULL
                AND lm.longitude IS NOT NULL AND lm.atualizada_em >= DATE_SUB( UTC_TIMESTAMP(), INTERVAL ' .
                self::LOCATION_MAX_AGE_SECONDS .
                ' SECOND
             )
             AND m.nascimento IS NOT NULL
    ...
    private function nearbyCount(string $memberId, float $latitude, float $longitude): int {
        [$latitudeDelta, $longitudeDelta] = ProximityConfig::boundingBoxDeltas($latitude);
        $statement = $this->db->prepare(
            'SELECT lm.membro_id, lm.latitude, lm.longitude
                ...
                WHERE lm.membro_id <> :member_id AND lm.localizacao_ativa = 1 AND lm.visivel = 1 AND lm.latitude IS NOT
                NULL AND lm.longitude IS NOT NULL AND lm.atualizada_em >= DATE_SUB( UTC_TIMESTAMP(), INTERVAL ' .
                self::LOCATION_MAX_AGE_SECONDS .
                ' SECOND
             )
             AND m.nascimento IS NOT NULL
        ...
        foreach ($statement->fetchAll(PDO::FETCH_ASSOC) as $row) {
            ...
            if (
                $this->distanceMetres($latitude, $longitude, $otherLatitude, $otherLongitude) <=
    ...
    private function resetAlert(string $memberId, int $nearbyCount): void {
        $statement = $this->db->prepare('UPDATE estado_app_membro
            SET alerta_proximidade_ativo = 0, total_proximidade = :nearby_count, atualizado_em = UTC_TIMESTAMP(6)
            WHERE membro_id = :member_id');
        $statement->execute(['nearby_count' => max(0, $nearbyCount), 'member_id' => $memberId]);
    }

    private function updateNearbyCount(string $memberId, int $nearbyCount): void {
        $statement = $this->db->prepare('UPDATE estado_app_membro
            SET total_proximidade = :nearby_count, atualizado_em = UTC_TIMESTAMP(6)
            WHERE membro_id = :member_id');
        $statement->execute(['nearby_count' => max(0, $nearbyCount), 'member_id' => $memberId]);
    }

    private function isBackground(string $memberId): bool {
        $statement = $this->db->prepare('SELECT em_background
    ...
    private function distanceMetres(float $latitude1, float $longitude1, float $latitude2, float $longitude2): float {
