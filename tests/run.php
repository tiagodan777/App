...
same(
    1,
    (int) $db->runSQL(
        'SELECT COUNT(*) FROM mensagens_apagadas WHERE mensagem_id = :id',
        ['id' => $id4]
    )->fetchColumn(),
...
$db->runSQL(
    "INSERT INTO notificacao (emissor_id, destinatario_id, tipo) VALUES (:a, :b, 'hey')",
    ['a' => $a, 'b' => $b]
);
...
$db->runSQL(
    'UPDATE membro_hoje SET expira_em = :date WHERE membro_id = :id',
    ['date' => '2000-01-01', 'id' => $a]
);
...
same(
    1,
    count($today->normaliseClothes([['type' => 'shirt'], ['type' => 'shirt']])),
    'Roupa sem tipos repetidos'
);
...
same(
    hash('sha256', $raw),
    $db->runSQL('SELECT token FROM token')->fetchColumn(),
...
$db->runSQL(
    'UPDATE membros SET email_verificado_em = :date WHERE id = :id',
...
$form = $member->prepareAccountForm(
    ['telefone' => '', 'email' => 'a@example.test'],
    ['contactos'],
    false
);
