window.MargotChatCamera = function ({ dialog, onFile, onError, gallery }) {
    ...
    function canvasFor(maxSize) {
        ...
        const scale = Math.min(1, maxSize / Math.max(live.videoWidth, live.videoHeight));
    ...
    async function startVideo() {
        ...
        try {
            ...
            if (!mime)
                throw new Error('Não foi possível encontrar um formato de vídeo compatível.');
        ...
        try {
            ...
            if (result.id) {
                ...
                while (offset < result.size) {
                    ...
                    const bytes = Uint8Array.from(atob(chunk.base64), (letter) =>
                        letter.charCodeAt(0)
                    );
                ...
                const bytes = Uint8Array.from(atob(result.base64), (letter) =>
                    letter.charCodeAt(0)
                );
                onFile(
