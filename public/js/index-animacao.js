(() => {
    ...
    function createGrid() {
        ...
        for (let row = 0; row < rows; row++) {
            for (let col = 0; col < cols; col++) {
                ...
                points.push({ baseX, baseY, nx: baseX * 0.003, ny: baseY * 0.003 });
            }
        }
    }
    ...
    // Gradiente: amarelo, azul, roxo e vermelho Margot.
    ...
    const palette = Array.from({ length: 256 }, (_, index) => getGradientColorRGB(index / 255));
    ...
    function draw(now) {
        ...
        for (let i = 0; i < points.length; i++) {
            ...
            const waveValue = (Math.sin(nx * 2.2 + time * 1.5) + Math.cos(ny * 2.2 + time * 1.5) + 2) * 0.25;
