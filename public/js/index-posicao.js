...
function inicializarFotos() {
    ...
    fotosCache = Array.from(fotos).map(function (elemento) {
        const top = Number(
            elemento.getAttribute('data-top')
        );

        const left = Number(
            elemento.getAttribute('data-left')
        );
...
/*
 * O websocket.js precisa de conseguir chamar esta função
 * sempre que entram, saem ou são atualizadas pessoas.
 */
...
function getTargetPan() {
    ...
    const minAllowedPanX =
        screenW - SAFE_RIGHT - maxLeft;

    const maxAllowedPanY =
        SAFE_TOP - minTop;

    const minAllowedPanY =
        screenH - SAFE_BOTTOM - maxTop;
...
function aplicarTransform() {
    ...
    for (const foto of fotosCache) {
        ...
        foto.elemento.style.transform =
            `translate3d(${posX}px, ${posY}px, 0) scale(${fotoScale})`;
    }
}
...
function iniciarInerciaFoto(
    ...
    function animar() {
        ...
        if (
            ...
            guardarPosicaoFoto(foto);
            return;
        }

        foto.left += velocidadeX;
        foto.top += velocidadeY;
...
function iniciarInerciaMapa() {
    ...
    function animar() {
        ...
        if (
            ...
            const cosA =
                Math.cos(mapVelRotation);

            const sinA =
                Math.sin(mapVelRotation);
...
function toqueComecouEmElementoIgnorado(alvo) {
...
function touchStart(evento) {
    if (
        toqueComecouEmElementoIgnorado(
            evento.target
        )
    ) {
        return;
...
function touchMove(evento) {
    ...
    for (
        ...
        if (ativos[touch.identifier]) {
            ativos[touch.identifier].x =
                touch.clientX;

            ativos[touch.identifier].y =
                touch.clientY;
        }
    }
