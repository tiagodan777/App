(function () {
    ...
    function metodoProtegido(metodo) {
        return metodosProtegidos.includes(String(metodo || 'GET').toUpperCase());
    }

    function mesmaOrigem(destino) {
        try {
            return new URL(destino, window.location.href).origin === window.location.origin;
        } catch (erro) {
            return false;
        }
    }
    ...
    function prepararFormularios(raiz) {
        ...
        if (raiz && typeof raiz.querySelectorAll === 'function') {
            formularios = formularios.concat(Array.from(raiz.querySelectorAll('form')));
        }
        formularios.forEach(function (formulario) {
            if (!metodoProtegido(formulario.method)) {
                return;
            }
            var campo = formulario.querySelector('input[name="_csrf"]');
            if (!campo) {
    ...
    if (typeof window.fetch === 'function') {
        ...
        window.fetch = function (entrada, opcoes) {
            ...
            var metodo = configuracao.method || (entrada instanceof Request ? entrada.method : 'GET');
            var destino = entrada instanceof Request ? entrada.url : String(entrada);
            if (metodoProtegido(metodo) && mesmaOrigem(destino)) {
                var headers = new Headers(
                    configuracao.headers || (entrada instanceof Request ? entrada.headers : undefined)
                );
    ...
    if (window.jQuery && typeof window.jQuery.ajaxPrefilter === 'function') {
        window.jQuery.ajaxPrefilter(function (opcoes, originais, xhr) {
            if (metodoProtegido(opcoes.type || opcoes.method) && mesmaOrigem(opcoes.url || window.location.href)) {
