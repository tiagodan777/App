(function (window, document, $) {
    ...
    $(function () {
        ...
        function libertarCapturaPonteiro(evento) {
            ...
            try {
                menuElemento.releasePointerCapture(
                    evento && evento.pointerId !== undefined ? evento.pointerId : ponteiroMenu
                );
            } catch (erro) {
                /*
         ...
         */
        function bloquearCliqueDepoisDeArrasto(evento) {
         ...
         */
        menuElemento.addEventListener('click', bloquearCliqueDepoisDeArrasto, true);
        ...
        $menu.on('pointerdown.margotMiniMenuSwipe', function (evento) {
            if (!aberto || evento.button > 0 || eCampoComInteracaoNativa(evento.target)) {
                gestoMenuAtivo = false;
                aArrastarMenu = false;
                ponteiroMenu = null;
                return;
        ...
        $menu.on('pointermove.margotMiniMenuSwipe', function (evento) {
            ...
            var distanciaX = evento.clientX - menuInicioX;
            var distanciaY = evento.clientY - menuInicioY;
            var absolutoX = Math.abs(distanciaX);
            var absolutoY = Math.abs(distanciaY);
            menuAtualY = evento.clientY;
            ...
            if (!aArrastarMenu) {
                ...
                if (menuElemento.setPointerCapture) {
                    try {
                        menuElemento.setPointerCapture(evento.pointerId);
                    } catch (erro) {
                        /*
                         * Safari pode
                         * recusar a
        ...
        $menu.on('pointerup.margotMiniMenuSwipe pointercancel.margotMiniMenuSwipe', function (evento) {
             ...
             * - gesto rápido.
             */
            if (distancia > 90 || velocidade > 0.5) {
                fecharMenu();
            } else {
                voltarMenu();
            }
