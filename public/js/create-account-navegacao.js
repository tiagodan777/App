(function (window, document, $) {
    ...
    if (modoEdicao) {
        ETAPAS = ETAPAS.filter(function (etapa) {
            return etapa !== '#legal';
        });
    }
    ...
    function guardarDadosNaSessao() {
        ...
        Object.keys(dados).forEach(function (chave) {
            if (
                ...
                chave !== 'password_atual' &&
                chave !== 'beta_invite_code'
            ) {
    ...
    function criarFormData() {
        ...
        formData.append('_csrf', String(config.csrfToken || ''));

        if (window.MargotPreferencias) {
            formData.append(
                'preferencia_localizacao',
                window.MargotPreferencias.obter('localizacao') === true ? '1' : '0'
            );
            formData.append(
                'preferencia_notificacoes',
                window.MargotPreferencias.obter('notificacoes') === true ? '1' : '0'
            );
        }
