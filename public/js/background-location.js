(function () {
    ...
    var plugin = capacitor && capacitor.Plugins ? capacitor.Plugins.BackgroundLocation : null;
    if (!plugin && capacitor && typeof capacitor.registerPlugin === 'function') {
        plugin = capacitor.registerPlugin('BackgroundLocation');
    }
    var geolocationPlugin = capacitor && capacitor.Plugins ? capacitor.Plugins.Geolocation : null;
    ...
    function aguardarPermissaoNotificacoesAndroid() {
        ...
        return new Promise(function (resolve) {
            ...
            var concluir = function () {
                ...
                terminou = true;
                window.removeEventListener('margot:notificacoes-permissao-concluida', concluir);
                resolve();
            };
            window.addEventListener('margot:notificacoes-permissao-concluida', concluir);
            if (window.margotNotificationPermissionFlowCompleted === true) {
                concluir();
            }
        });
    ...
    function publicarPermissaoLocalizacaoAndroid(granted, estado) {
        ...
        window.dispatchEvent(
            new CustomEvent('margot:localizacao-permissao-concluida', {
                detail: { granted: granted === true, status: estado || null }
            })
        );
    }
    ...
    function garantirPermissaoLocalizacaoAndroid() {
        ...
        androidLocationPermissionPromise = aguardarPermissaoNotificacoesAndroid()
            ...
            .then(function (estado) {
                ...
                return geolocationPlugin.requestPermissions({ permissions: ['location'] }).then(function (novoEstado) {
                    return { granted: permissaoLocalizacaoConcedida(novoEstado), status: novoEstado };
                });
            })
            .then(function (resultado) {
                androidLocationPermissionPromise = null;
                publicarPermissaoLocalizacaoAndroid(resultado.granted, resultado.status);
                return resultado;
            })
            .catch(function (erro) {
                androidLocationPermissionPromise = null;
                console.error('Não foi possível pedir a permissão de localização:', erro);
    ...
    async function pedirTokenBackground() {
        ...
        var resposta = await fetch(String(window.backgroundLocationTokenUrl || '/background-location-token/'), {
            method: 'POST',
        ...
        if (!resposta.ok || !dados.success || !dados.token) {
            throw new Error(dados.message || 'Não foi possível ativar a localização em segundo plano.');
        }
    ...
    function precisaDasDefinicoes(resultado) {
        ...
        if (resultado.requiresSettings === true || resultado.requires_settings === true) {
            return true;
        }
        var autorizacao = String(
            resultado.authorization || resultado.authorizationStatus || resultado.authorization_status || ''
        ).toLowerCase();
        var permissao = String(
            resultado.permission || resultado.permissionStatus || resultado.permission_status || ''
        ).toLowerCase();
        return (
    ...
    function localizacaoAtiva(resultado) {
        return !!(
            resultado &&
            (resultado.active === true || resultado.isActive === true || resultado.is_active === true)
        );
    }

    function tokenGuardado(resultado) {
        return !!(resultado && (resultado.token_stored === true || resultado.tokenStored === true));
    }
    ...
    function fecharAviso() {
        var aviso = document.getElementById('margot-background-location-overlay');
        if (aviso) {
            aviso.remove();
        }
    }
    ...
    function mostrarAvisoDefinicoes(forcar) {
        ...
        if (document.getElementById('margot-background-location-overlay')) {
            return;
        }
        var chave = 'margot-background-location-aviso';
        var ultimaApresentacao = Number(localStorage.getItem(chave) || 0);
        ...
        fundo.setAttribute('aria-labelledby', 'margot-background-location-title');
        ...
        abrirDefinicoes.className = 'margot-background-location-settings';
        ...
        abrirDefinicoes.addEventListener('click', async function () {
            ...
            try {
                ...
                console.error('Não foi possível abrir as definições:', erro);
            }
        });
    ...
    async function iniciar(forcarAviso) {
        ...
        inicializacao = (async function () {
            try {
                ...
                return { available: true, active: false, error: String(erro && erro.message ? erro.message : erro) };
            } finally {
                inicializacao = null;
            }
        })();
    ...
    if (pluginDisponivel() && typeof plugin.addListener === 'function') {
        var renovarAutorizacao = function () {
            ...
            renovarToken().catch(function (erro) {
                console.error('Não foi possível renovar a autorização da localização:', erro);
            });
        };
        ['backgroundLocationAuthorizationExpired', 'backgroundLocationTokenExpired'].forEach(function (evento) {
            plugin.addListener(evento, renovarAutorizacao);
        });
        plugin.addListener('backgroundLocationAuthorizationChanged', function () {
            if (localizacaoPermitida()) {
                iniciar(false);
            }
        });
