(function (window, document) {
    ...
    function renderProfile(status) {
        ...
        if (add) {
            ...
            add.setAttribute(
                'aria-label',
                validStatus(profileStatus) ? 'Editar a tua nota e roupa de hoje' : 'Adicionar nota ou roupa de hoje'
            );
        }
    }
    ...
    function renderPieceGrid() {
        ...
        PIECES.forEach(function (piece) {
            ...
            icon.appendChild(
                createGarmentIcon(piece.type, chosen && chosen.color ? chosen.color : '', 'hoje-peca-garment')
            );
    ...
    function renderColorGrid() {
        ...
        if (preview) {
            clearNode(preview);
            preview.appendChild(
                createGarmentIcon(activePiece, chosen && chosen.color ? chosen.color : '', 'hoje-cores-peca-garment')
            );
        }
    ...
    function bindEditorSwipe(editor) {
        ...
        if (!sheet) return;
        var state = { tracking: false, dragging: false, startY: 0, currentY: 0, startedAt: 0 };
        function reset() {
    ...
    function deleteEditor() {
        ...
        window
            ...
            .catch(function (error) {
                showMessage(
                    error && error.message ? error.message : 'Não foi possível apagar. Tenta novamente.',
                    'erro'
                );
            })
            .finally(function () {
