(function (window, document) {
    ...
    function guardarEntradaPendente() {
        try {
            window.sessionStorage.setItem(CHAVE_ENTRADA, String(Date.now()));
        } catch (erro) {
    ...
    function consumirEntradaPendente() {
        var valor = '';
        try {
            valor = window.sessionStorage.getItem(CHAVE_ENTRADA) || '';
    ...
    function animarEntrada() {
        ...
        if (movimentoReduzido() || !document.body || !document.body.animate) {
            return;
        }
        cancelarAnimacaoAtual();
        try {
            animacaoAtual = document.body.animate(
                [
                    { transform: 'translate3d(14px, 0, 0)', opacity: 0.86 },
                    { transform: 'translate3d(0, 0, 0)', opacity: 1 }
                ],
                { duration: DURACAO_ENTRADA, easing: 'cubic-bezier(.22, .8, .28, 1)' }
            );
    ...
    async function animarSaida(voltar) {
        ...
        try {
            animacaoAtual = document.body.animate(
                [
                    { transform: 'translate3d(0, 0, 0)', opacity: 1 },
                    { transform: voltar ? 'translate3d(16px, 0, 0)' : 'translate3d(-16px, 0, 0)', opacity: 0.82 }
                ],
                { duration: DURACAO_SAIDA, easing: 'cubic-bezier(.22, .8, .28, 1)', fill: 'forwards' }
            );
    ...
    document.addEventListener('click', function (evento) {
        ...
        sair(destino.href, link.hasAttribute('data-margot-voltar'));
    });
    ...
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', animarEntrada, { once: true });
    } else {
        animarEntrada();
    }
    window.MargotDocumentNavigation = { navigate: sair, back: voltar };
})(window, document);
