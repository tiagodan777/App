(function (window, document) {
    ...
    function erroFoiCancelamento(erro) {
        ...
        var mensagem = String(erro && erro.message ? erro.message : '').toLowerCase();
        return ['OS-PLUG-CAMR-0006', 'OS-PLUG-CAMR-0020'].indexOf(codigo) !== -1 || mensagem.indexOf('cancel') !== -1;
    }
    ...
    function erroFoiPermissao(erro) {
        ...
        var mensagem = String(erro && erro.message ? erro.message : '').toLowerCase();
        return (
            ['OS-PLUG-CAMR-0003', 'OS-PLUG-CAMR-0005'].indexOf(codigo) !== -1 || mensagem.indexOf('permission') !== -1
        );
    }
    ...
    function mostrarErro(idInput, erro) {
        ...
        var seletorErro =
            idInput === 'perfil-input-fotos' ? '#perfil-fotos-erro' : idInput === 'chat-media' ? '#chat-erro' : '';
        var elementoErro = seletorErro ? document.querySelector(seletorErro) : null;
        if (elementoErro) {
    ...
    function caminhoWebDoResultado(resultado) {
        ...
        if (resultado && resultado.uri && window.Capacitor && typeof window.Capacitor.convertFileSrc === 'function') {
    ...
    function resultadoEVideo(resultado) {
        return Boolean(resultado && (resultado.type === 1 || String(resultado.type).toLowerCase() === 'video'));
    }

    function mimeDoResultado(resultado, blob, eVideo) {
        var formato = String(
            resultado && resultado.metadata && resultado.metadata.format ? resultado.metadata.format : ''
        )
            .toLowerCase()
            .replace(/^[.]/, '');
        if (blob.type) {
    ...
    async function converterResultadoEmFicheiro(resultado, indice) {
        ...
        if (!caminho) {
            throw new Error('O ficheiro escolhido não tem um caminho acessível.');
        }
        var resposta = await window.fetch(caminho);
        if (!resposta.ok) {
            throw new Error('Não foi possível ler o ficheiro escolhido.');
        }
        ...
        var nome = 'margot-' + (eVideo ? 'video-' : 'foto-') + Date.now() + '-' + (indice + 1) + '.' + extensao;
        if (blob.type !== mime) {
            blob = blob.slice(0, blob.size, mime);
        }
        return new window.File([blob], nome, { type: mime, lastModified: Date.now() });
    }

    async function converterResultados(resultados) {
        var ficheiros = [];
        for (var indice = 0; indice < resultados.length; indice += 1) {
            ficheiros.push(await converterResultadoEmFicheiro(resultados[indice], indice));
        }
        return ficheiros;
    }
    ...
    async function executar(idInput, operacao) {
        ...
        try {
            ...
            colocarFicheirosNoInput(idInput, ficheiros);
        } catch (erro) {
    ...
    function tirarFotoDePerfil() {
        executar('perfil-input-fotos', async function () {
    ...
    function escolherFotosDePerfil() {
        executar('perfil-input-fotos', async function () {
            var selecao = await obterCamera().chooseFromGallery({
                quality: 90,
                correctOrientation: true,
                includeMetadata: true,
                mediaType: 0,
            ...
            return selecao && Array.isArray(selecao.results) ? selecao.results : [];
        });
    }

    function escolherAnexo(idInput) {
        executar(idInput, async function () {
            var selecao = await obterCamera().chooseFromGallery({
                quality: 90,
                correctOrientation: true,
                includeMetadata: true,
    ...
    document.addEventListener(
        ...
        function (evento) {
            ...
            var botaoTirarFoto = evento.target.closest('#perfil-tirar-foto');
            var botaoGaleria = evento.target.closest(
                '#perfil-escolher-fotos, ' + '#perfil-abrir-galeria-camera, ' + '.perfil-foto-placeholder'
            );
            var botaoAnexo = evento.target.closest('label[for="chat-media"], ' + 'label[for="mini-menu-media"]');
            if (!botaoTirarFoto && !botaoGaleria && !botaoAnexo) {
                return;
            }
            ...
            escolherAnexo(botaoAnexo.getAttribute('for'));
        },
        true
    );
})(window, document);
