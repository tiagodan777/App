(function (window, document) {
    ...
    function notificacoesNativas() {
        ...
        if (!push || typeof push.isNative !== 'function' || typeof push.isAvailable !== 'function') {
            return null;
        }
        if (!push.isNative() || !push.isAvailable()) {
            return null;
        }
        return push;
    }
    ...
    if (notificacoesNativas()) {
        ...
        if (typeof window.margotNotificationPermissionFlowCompleted !== 'boolean') {
    ...
    function concluirFluxoPermissaoNotificacoes() {
        ...
        window.dispatchEvent(new CustomEvent('margot:notificacoes-permissao-concluida'));
    }
    ...
    function removerAvisoTopo(aviso, imediato) {
        if (!aviso || !aviso.isConnected) {
            return;
        }
        var timer = Number(aviso.dataset.removerTimer || 0);
        if (timer) {
    ...
    function limitarAvisosTopo() {
        ...
        var itens = Array.from(avisos.querySelectorAll('.hey-aviso, .mensagem-aviso'));
        while (itens.length >= 3) {
            var primeiro = itens.shift();
            removerAvisoTopo(primeiro, true);
        }
    }

    function ativarSwipeAviso(aviso) {
        if (!aviso) {
            return;
        }
        var gesto = null;
        ...
        aviso.addEventListener('pointermove', function (evento) {
            ...
            if (dy < 0) {
                ...
                var opacidade = Math.max(0.18, 1 - Math.abs(deslocamento) / 110);
                aviso.style.transition = 'none';
                aviso.style.transform = 'translateY(' + deslocamento + 'px) scale(0.985)';
    ...
    function mostrarAviso(opcoes) {
        ...
        if (dados.foto) {
            ...
            var conteudoSimbolo = dados.tipo === 'erro' ? '!' : '👋';
            if (dados.icone === 'seta-cima') {
                conteudoSimbolo = '↑';
            }
    ...
    async function pedirPermissao() {
        ...
        if (push) {
            try {
                ...
                console.warn('Não foi possível pedir permissão nativa para notificações.', falha);
            }
            return;
        }
    ...
    function prepararPedidoPermissao() {
        ...
        if (window.disableNotifications || !('Notification' in window) || Notification.permission !== 'default') {
            concluirFluxoPermissaoNotificacoes();
            return;
        }
        ...
        try {
            ...
            estado.notificacoes = estado.notificacoes.map(function (item) {
                if (item.direcao === 'recebido') {
                    return Object.assign({}, item, { lida: true });
                }
                return item;
            });
            definirContador(0);
