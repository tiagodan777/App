...
window.MargotLocationTracker = function (actions) {
        ...
        if (isNativeApp()) {
            ...
            if (isAndroidNativeApp()) {
                ...
                ensureAndroidLocationPermission(nativeGeolocation).then(function (granted) {
                    if (!granted || window.disableLocationTracking || document.visibilityState !== 'visible') {
    ...
    function startNativeLocationWatch(nativeGeolocation) {
        ...
        nativeGeolocation
            .watchPosition(getLocationOptions(), function (position, error) {
                if (generation !== locationWatchGeneration || window.disableLocationTracking) {
                    return;
                }
                if (error) {
            ...
            .then(function (watchId) {
                if (generation !== locationWatchGeneration || window.disableLocationTracking) {
                    locationWatchStarting = false;
                    return nativeGeolocation.clearWatch({ id: String(watchId) }).catch(function (error) {
    ...
    function requestCurrentLocation() {
        if (window.disableLocationTracking || document.visibilityState !== 'visible' || locationRequestPending) {
            return;
        }
        var nativeGeolocation = getNativeGeolocation();
        if (isNativeApp()) {
    ...
    function ensureAndroidLocationPermission(nativeGeolocation) {
        ...
        androidLocationPermissionPromise = nativeGeolocation
            ...
            .catch(function (error) {
                ...
                androidLocationPermissionConfirmed = false;
                limparMapaLocal();
                console.warn('Não foi possível verificar a permissão de localização.', error);
                return false;
            });
        return androidLocationPermissionPromise;
    }
    function getLocationOptions() {
        return { enableHighAccuracy: true, maximumAge: LOCATION_MAX_AGE, timeout: LOCATION_TIMEOUT };
    }
    function startLocationRefresh() {
        clearLocationRefreshTimer();
        locationRefreshTimer = window.setInterval(function () {
            if (document.visibilityState !== 'visible' || window.disableLocationTracking) {
                return;
            }
    ...
    function stopLocationTracking() {
        ...
        } else if (watchId !== null && watchProvider === 'web' && navigator.geolocation) {
    ...
    function handleLocationSuccess(position) {
        ...
        if (lastSentLatitude !== null && !passouTempo && distancia < LOCATION_MIN_DISTANCE) {
            return;
        }
        sendLastKnownLocation();
    }
    ...
    function handleLocationError(error) {
        ...
        var errorMessage = String(error && error.message ? error.message : '').toLowerCase();
        ...
        if (permissionDenied) {
            ...
            if (
                lastKnownLocation ||
                (locationTrackingStartedAt > 0 && Date.now() - locationTrackingStartedAt < LOCATION_STARTUP_GRACE)
            ) {
                console.warn(
        ...
        if (lastLocationErrorAt > 0 && Date.now() - lastLocationErrorAt < LOCATION_ERROR_COOLDOWN) {
            return;
        }
