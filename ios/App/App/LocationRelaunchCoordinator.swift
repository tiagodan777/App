 ...
 */
  final class LocationRelaunchCoordinator: NSObject, CLLocationManagerDelegate {
  ...
  func start() {
     ...
     */
  guard readVisibility(), readToken() != nil, CLLocationManager.locationServicesEnabled(),
  ...
  manager.startMonitoringSignificantLocationChanges()
     ...
     */
    if let location = manager.location, isUsableLocation(location, maxAge: 180) {
      considerSending(location, force: true)
      return
    }
    /*
     ...
     */
  if let pending = readPendingLocation(maxAge: 180) { considerSending(pending, force: true) }
  }
  ...
  func locationManagerDidChangeAuthorization(_ manager: CLLocationManager) {
    guard !stopped else { return }
    guard manager.authorizationStatus == .authorizedAlways, readVisibility(), readToken() != nil else {
      stop()
      return
    }
    manager.allowsBackgroundLocationUpdates = true
  ...
  func locationManager(_ manager: CLLocationManager, didFailWithError error: Error) {
     ...
     */
  if locationError?.code != .locationUnknown { print("[MARGOT LOCATION RELAUNCH] " + error.localizedDescription) }
  }

  private func considerSending(_ location: CLLocation, force: Bool) {
    guard !stopped, readVisibility(), readToken() != nil, isUsableLocation(location, maxAge: 180) else { return }
    if !force {
    ...
    performSend(location)
  }

  private func performSend(_ location: CLLocation) {
    guard !stopped, !sending, readVisibility(), let token = readToken() else { return }
    sending = true
    ...
    let payload: [String: Any] = [
      ...
      "active": true, "visible": true, "token": token,
    ]
    ...
    session.dataTask(with: request) { [weak self] _, response, error in
      ...
      DispatchQueue.main.async { self?.finishSend(location, statusCode: statusCode, error: error) }
    }.resume()
  }

  private func finishSend(_ location: CLLocation, statusCode: Int?, error: Error?) {
    sending = false
    if statusCode == 401 {
      deleteToken()
    ...
    if let statusCode = statusCode, (200...299).contains(statusCode) {
      ...
      scheduleRetry()
      if let error = error {
        print("[MARGOT LOCATION RELAUNCH] " + error.localizedDescription)
      } else if let statusCode = statusCode {
        print("[MARGOT LOCATION RELAUNCH] HTTP \(statusCode)")
      }
    }
    endBackgroundTask()
    if let pending = pendingLocation, isUsableLocation(pending, maxAge: 180), statusCode != nil,
      statusCode.map({ (200...299).contains($0) }) == true
    {
      pendingLocation = nil
      considerSending(pending, force: false)
    }
  }

  private func scheduleRetry() {
    guard !stopped, retryWorkItem == nil, let location = pendingLocation, isUsableLocation(location, maxAge: 180) else {
      return
    }
    ...
    let workItem = DispatchWorkItem { [weak self] in
      ...
      guard let location = self.pendingLocation, self.isUsableLocation(location, maxAge: 180) else {
        self.pendingLocation = nil
        self.deletePendingLocation()
        return
      }
      self.performSend(location)
    }
  ...
  private func beginBackgroundTask() {
    ...
    backgroundTask = UIApplication.shared.beginBackgroundTask(withName: "MargotLocationRelaunch") { [weak self] in
  ...
  private func readPendingLocation(maxAge: TimeInterval) -> CLLocation? {
    ...
    guard abs(date.timeIntervalSinceNow) <= maxAge else {
      deletePendingLocation()
      return nil
    }
    return CLLocation(
      coordinate: CLLocationCoordinate2D(latitude: latitude, longitude: longitude), altitude: 0,
      horizontalAccuracy: accuracy, verticalAccuracy: -1, timestamp: date)
  }
  ...
  private func readToken() -> String? {
    ...
    guard cleanToken.range(of: "^[A-Fa-f0-9]{64}$", options: .regularExpression) != nil else { return nil }
    return cleanToken
  }
