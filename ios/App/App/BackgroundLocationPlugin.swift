...
public final class BackgroundLocationPlugin: CAPPlugin, CAPBridgedPlugin, CLLocationManagerDelegate {
  ...
  private let visibilityKey = "margot.background-location.visible"
  ...
  private let networkQueue = DispatchQueue(label: "com.margot.background-location.network")
  ...
  manager.distanceFilter = kCLDistanceFilterNone
    ...
    presenceVisible = readVisibility()
    retryLocation = readPendingLocation()
    latestLocation = retryLocation ?? manager.location
    networkMonitor.pathUpdateHandler = { [weak self] path in
      guard path.status == .satisfied else { return }
      DispatchQueue.main.async { self?.retryWhenConnectivityReturns() }
    }
    ...
    if !presenceVisible {
      queueHiddenPresence()
    } else if let retryLocation = retryLocation {
      queueLocation(retryLocation, force: true)
    }
  }
  ...
  @objc public func start(_ call: CAPPluginCall) {
    ...
    guard CLLocationManager.locationServicesEnabled() else {
      if !presenceVisible { queueHiddenPresence() }
      call.resolve(statusData())
      return
    }
    switch manager.authorizationStatus {
    case .notDetermined:
      pendingStartCall?.reject("Foi iniciado um novo pedido de localização.")
  ...
  @objc public func stop(_ call: CAPPluginCall) {
    ...
    if readToken() != nil { queueHiddenPresence() }
    stopMonitoring(removeToken: true)
    call.resolve(statusData())
  }

  @objc public func status(_ call: CAPPluginCall) { call.resolve(statusData()) }
  ...
  if !presenceVisible { return }
     ...
     */
    if retryLocation != nil, !sending {
      retryLocation = newest(location, retryLocation)
      if let retryLocation = retryLocation { queueLocation(retryLocation, force: true) }
      return
    }
  ...
  public func locationManager(_ manager: CLLocationManager, didVisit visit: CLVisit) {
    ...
    latestLocation = location
    if presenceVisible { queueLocation(location, force: true) }
  }

  public func locationManager(_ manager: CLLocationManager, didFailWithError error: Error) {
    let locationError = error as? CLError
    if locationError?.code == .denied { stopMonitoring(removeToken: false) }
    /*
  ...
  private func configureMonitoring() {
    guard manager != nil, readToken() != nil, CLLocationManager.locationServicesEnabled() else {
  ...
  manager.allowsBackgroundLocationUpdates = true
      ...
      monitoring = true
    case .authorizedWhenInUse: startForegroundLocationIfPossible()
    default: stopMonitoring(removeToken: false)
    }
  }
  ...
  private func requestCurrentLocationIfPossible(allowBackground: Bool) {
    guard readToken() != nil else { return }
    let state = UIApplication.shared.applicationState
    switch manager.authorizationStatus {
    case .authorizedAlways: if state == .active || allowBackground { manager.requestLocation() }
    case .authorizedWhenInUse: if state == .active { manager.requestLocation() }
    default: break
    }
  }

  private func sendBestAvailableLocationOrRequest(allowBackground: Bool) {
    guard presenceVisible, readToken() != nil else { return }
    if let retryLocation = retryLocation {
      queueLocation(retryLocation, force: true)
      return
    }
    if let latestLocation = latestLocation, isUsableLocation(latestLocation, maxAge: 900) {
      queueLocation(latestLocation, force: true)
      return
    }
    if let managerLocation = manager.location, isUsableLocation(managerLocation, maxAge: 900) {
      latestLocation = managerLocation
      queueLocation(managerLocation, force: true)
      return
    }
    requestCurrentLocationIfPossible(allowBackground: allowBackground)
  }
  ...
  private func queueLocation(_ location: CLLocation, force: Bool) {
    ...
    performLocationSend(location)
  }

  private func performLocationSend(_ location: CLLocation) {
    guard let token = readToken(), presenceVisible, !sending else { return }
    /*
  ...
  private func finishLocationSending(_ location: CLLocation, statusCode: Int?, error: Error?) {
    sending = false
    if handleUnauthorized(statusCode) { return }
    if let statusCode, (200...299).contains(statusCode) {
      ...
      retryAttempt = 0
      deletePendingLocation()
      notifyListeners("backgroundLocationUpdated", data: locationPayload(location))
    } else if presenceVisible {
      retryLocation = newest(location, retryLocation)
      savePendingLocation(retryLocation ?? location)
      scheduleRetry()
      notifySendError(statusCode: statusCode, error: error)
    }
    processPendingAfterSend()
  }

  private func finishHiddenSending(statusCode: Int?, error: Error?) {
    sending = false
    if handleUnauthorized(statusCode) { return }
    if let statusCode, (200...299).contains(statusCode) {
      retryHidden = false
  ...
  private func handleUnauthorized(_ statusCode: Int?) -> Bool {
    ...
    stopMonitoring(removeToken: true)
    notifyListeners("backgroundLocationTokenExpired", data: ["expired": true])
    return true
  }
  ...
  private func scheduleRetry() {
    guard retryHidden || retryLocation != nil, retryWorkItem == nil else { return }
    ...
    let workItem = DispatchWorkItem { [weak self] in
      ...
      if !self.presenceVisible {
        if self.retryHidden || self.readToken() != nil { self.performHiddenSend() }
        return
      }
      self.retryHidden = false
      if let location = self.retryLocation {
        self.performLocationSend(location)
      } else {
        self.sendBestAvailableLocationOrRequest(allowBackground: true)
      }
    }
  ...
  private func retryWhenConnectivityReturns() {
    ...
    retryAttempt = 0
    if sending {
      if !presenceVisible { hiddenSyncPending = true }
      return
    }
    if !presenceVisible {
      retryHidden = true
    ...
    retryHidden = false
    if let retryLocation = retryLocation {
      performLocationSend(retryLocation)
      return
    }
    sendBestAvailableLocationOrRequest(allowBackground: true)
  }

  private func newest(_ first: CLLocation, _ second: CLLocation?) -> CLLocation {
    guard let second = second else { return first }
    return first.timestamp >= second.timestamp ? first : second
  }

  private func isUsableLocation(_ location: CLLocation, maxAge: TimeInterval) -> Bool {
  ...
  private func readVisibility() -> Bool {
    guard UserDefaults.standard.object(forKey: visibilityKey) != nil else { return true }
    return UserDefaults.standard.bool(forKey: visibilityKey)
  }
  ...
  private func send(payload: [String: Any], token: String, completion: ((Int?, Error?) -> Void)? = nil) {
    ...
    if UIApplication.shared.applicationState != .active {
      backgroundTask = UIApplication.shared.beginBackgroundTask(withName: "MargotLocationUpload")
    }
    ...
    guard let body = try? JSONSerialization.data(withJSONObject: authenticatedPayload) else {
  ...
  private func validToken(_ value: String?) -> String? {
    ...
    return token.range(of: "^[A-Fa-f0-9]{64}$", options: .regularExpression) == nil ? nil : token
  }
  ...
  private func deleteToken() { SecItemDelete(keychainQuery() as CFDictionary) }
}
