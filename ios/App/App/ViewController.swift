...
@objc(MargotHapticsPlugin) public final class MargotHapticsPlugin: CAPPlugin, CAPBridgedPlugin {
  public let identifier = "MargotHapticsPlugin"
  public let jsName = "MargotHaptics"
  public let pluginMethods: [CAPPluginMethod] = [CAPPluginMethod(name: "play", returnType: CAPPluginReturnPromise)]
  ...
  private func playPattern(_ type: String) {
    switch type {
    case "heySent": impact(.light, intensity: 0.65)
    case "heyReceived":
      impact(.rigid, intensity: 0.9)
      after(0.10) { [weak self] in self?.impact(.medium, intensity: 0.78) }
    case "connection":
      let generator = UINotificationFeedbackGenerator()
      generator.prepare()
      generator.notificationOccurred(.success)
      after(0.12) { [weak self] in self?.impact(.heavy, intensity: 1.0) }
      after(0.28) { [weak self] in self?.impact(.rigid, intensity: 0.96) }
    default: impact(.medium, intensity: 0.72)
    }
  }
  ...
  private func impact(_ style: UIImpactFeedbackGenerator.FeedbackStyle, intensity: CGFloat) {
    ...
    generator.prepare()
    if #available(iOS 13.0, *) { generator.impactOccurred(intensity: intensity) } else { generator.impactOccurred() }
  }
